...
import { ShoppingCart, Search, PlusCircle, Download, Edit2, Trash2, X, Save, FileText, Hash, Package, Eye } from 'lucide-react';
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden text-sm">
                <table className="w-full text-left">
                    <thead className="bg-gray-50 text-gray-500 text-[10px] font-black uppercase tracking-widest">
                        <tr>
                            <th className="px-6 py-4">Facture / Bon (Date)</th>
                            <th className="px-6 py-4">Fournisseur</th>
                            <th className="px-6 py-4">Détails Lignes</th>
                            <th className="px-6 py-4 text-right">Montant Global TTC</th>
                            <th className="px-6 py-4 text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            <tr><td colSpan={5} className="p-10 text-center text-gray-400 italic">Chargement des factures...</td></tr>
                        ) : filtered.length === 0 ? (
                            <tr><td colSpan={5} className="p-10 text-center text-gray-400 font-bold">Aucune facture trouvée</td></tr>
                        ) : filtered.map(inv => (
                            <tr key={inv.id} className="hover:bg-emerald-50/20 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <button
                                        ...
                                        className="flex items-center gap-2 bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg border border-blue-100 hover:bg-blue-100 transition-colors font-bold text-[10px] uppercase tracking-widest"
                                    >
                                        <Eye size={16} />
                                        Voir Détails ({inv.items.length})
                                    </button>
                                </td>

                                <td className="px-6 py-4 align-top text-right">
                                    <div className="flex flex-col items-end">
                                        <span className="px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 font-black text-sm border border-emerald-100">
                                            {inv.total_ttc?.toLocaleString('fr-MA')} DH
                                        </span>
                                        <span className="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-wider">
                                            HT: {inv.total_ht?.toLocaleString('fr-MA')} DH
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4 align-top text-right">
                                    <div className="flex items-center justify-end gap-2 transition-opacity">
                                        {inv.scan_contract && (
                                            <button onClick={() => handleViewContract(inv.scan_contract!)} title="Voir Contrat Sécurisé" className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors border border-emerald-100">
                                                <FileText size={14} /> Voir Scan
                                            </button>
                                        )}
                                        <button onClick={() => handleEdit(inv)} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors border border-transparent hover:border-indigo-100" title="Modifier la facture">
                                            <Edit2 size={18} />
                                        </button>
                                        <button onClick={() => handleDelete(inv.id)} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors border border-transparent hover:border-rose-100">
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                                </td>
                            ...
                            <form id="editInvoiceForm" onSubmit={handleSubmit(onSubmitUpdate)} className="space-y-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Nouv. Scan (Opt.) {editingInvoice?.scan_contract && "✅ Actif"}</label>
                                            <input
                                                type="file"
                                                accept=".pdf,.jpg,.jpeg,.png"
                                                {...register('scan_contract')}
                                                className="block w-full text-sm text-gray-500 file:mr-3 file:py-2.5 file:px-4 file:rounded-l-xl file:border-0 file:text-[10px] file:font-black file:uppercase file:bg-gray-100 file:text-gray-600 hover:file:bg-gray-200 bg-white border border-gray-200 rounded-xl focus:border-emerald-600 focus:ring-2 focus:ring-emerald-50 transition-all outline-none"
                                            />
                                        </div>
                                    </div>
                                </div>
